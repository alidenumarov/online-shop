...
.Adapter<AdapterBucket.BucketViewHolder>() {
    ...
    override fun onBindViewHolder(holder: BucketViewHolder, position: Int) {
        ...
        holder.decreaseCountIV.setOnClickListener{
            ...
            if (curCount > 1) {
                curCount -= 1
                addOrRemoveCountInBucket(bucketProducts[position], curCount, holder)
    ...
    private fun addOrRemoveCountInBucket(product : Product, curCount : Int, holder: BucketViewHolder): Boolean {
        var result = true
        val db = holder.db.getReference("bucket_items")
        db.child(holder.userEmail).child(product.id.toString()).child("count_in_bucket").setValue(curCount).
        addOnSuccessListener {
